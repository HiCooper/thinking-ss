...
| 路径 | 说明 |
|---|---|
| `market-dashboard/` | 本地看板（React + Vite + ECharts）。两个视图：大盘趋势 / 我的持仓 |
...
- **大盘趋势**：底部固定指数条（上证/深成/创业板/科创50/北证50）+ 8 张实时报价卡（含黄金/白银/WTI/QVIX，带日内迷你分时）+ 趋势状态横幅 + 4 张摘要卡 + 6 张日频图
- **我的持仓**：4 张总览卡 + 明细表（可排序、分组筛选、以 0 为中心的发散微条）+ **账户收益走势** + 分组结构图 + 个股盈亏排行（每天收盘后记录一笔收益，只记真实值、不回填）

完整设计、数据口径与边界见 **[`market-dashboard/README.md`](market-dashboard/README.md)**。

### 界面

**大盘趋势** —— 底部固定指数条（上证/深成/创业板/科创50/北证50）+ 8 张实时报价卡（A50 / KOSPI / 纳指期货 / 恒生科技 / COMEX 黄金 / COMEX 白银 / WTI 原油 / QVIX，带日内迷你分时）+ 趋势状态横幅 + 4 张摘要卡 + 6 张日频图：

![大盘趋势](img/大盘看板.png)

**我的持仓** —— 4 张总览卡 + 明细表（可排序、分组筛选、以 0 为中心的发散微条）+ 账户收益走势 + 分组结构图 + 个股盈亏排行 + 底部合计条：

![我的持仓](img/持仓看板示例.png)

> 持仓看板这张用的是 `holdings.example.md` 的**示例数据**（3 只持仓，份额与成本全是编造的）。
> 账户收益走势同样是**为截图造出来的示例**（成本 39,600 元、市值围绕 41,000 元合成 7 个交易日）——
