...
| 区块 | 内容 |
|---|---|
| **总览卡（5 张）** | 持仓市值（含今日盈亏）、持仓成本（含回本需涨／可回撤）、浮动盈亏（含最大亏损与最大盈利）、今日盈亏（含涨/跌只数与占市值比）、价格口径（实时 N/总只数 + 报价时间 + 快照回退只数） |
...
| **明细表** | 可排序（点表头，默认按浮动盈亏从小到大＝最惨在前）+ 分组筛选；列含份额/成本/现价/今日/市值/浮动盈亏/盈亏率（带**以 0 为中心的发散微条**）/距成本 |
...
| 本地实时 | `GET /api/holdings` | **22 只的现价**（一次批量请求） | 逐只回退到 `snapshot_price`，标注「快照」 |
...
├── public/holdings.json        # 持仓快照：份额与成本（由 holdings.md 生成）
...
└── src/
    ...
    ├── components/HoldingsSummary.tsx        # 持仓总览卡（5 张）
...
- **持仓快照价只用于降级**：无实时接口时页面显示的是 `holdings.md` 快照时点的价格，不是当前价，请以卡片上的「价格口径」为准。
- 数据为第三方接口，**仅供研究**。
