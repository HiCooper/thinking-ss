...
**指数信息条（吸顶）**：**上证 / 深成 / 创业板 / 科创50 / 北证50** 五个指数各带实时价与涨跌幅，**固定在屏幕最顶部、铺满 100% 宽**，向下滚动时始终可见（5 等列；<1080px 降 3 列、<640px 降 2 列）。
...
> **吸顶信息条怎么做的**：它由 `LiveStrip` 用 `createPortal` 渲染进 `index.html` 里 `#root` **之前**的
> `#ticker-root`（body 直属子元素），配合 `position: sticky; top: 0` —— 这样才对**整页滚动**生效；
> 若留在面板卡片内部，sticky 会被卡片的盒子限制住（面板仅 ~200px 高），几乎不吸顶。
> 背景层 `#ticker-root` 满宽，内层 `.live-indices` 限宽 1320px 与下方卡片左右对齐。
...
> 因此页面只有两处内容是实时的：**吸顶指数条**与**实时行情面板**（同一个 30 秒轮询、同源 `/api/spot`）；其余皆为日频历史。
