...
**顶部实时面板**（每 30 秒自动刷新，可暂停）：两市成交额（含沪/深/北证50 分项）、富时中国 A50 期货、韩国 KOSPI、**纳指期货（NQ）**。每张报价卡片的排版是「**数值 + 涨跌幅同一行**（涨跌幅紧跟数值），**下一行占满内容宽度的内联 SVG 迷你分时图**」（高 20px，末值 ≥ 昨结→红、否则绿，hover 显示「A50 分时 17:01→21:43 +0.31%」这样的口径与区间涨跌幅）；卡片右上角是各自报价时点，顶部标注「盘前 / 交易中 / 午间休市 / 已收盘」。
