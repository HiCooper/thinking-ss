...
| **图 3 账户收益走势** | **账户级汇总**（不含逐只），每个点是一天收盘后的实际持仓；可切「金额（元）/ 收益率（%）」。**只记真实值、不回填** —— 详见下节 |
...
- **今日盈亏率除的是「昨收市值」，不是当前市值**（`todayPnlPct = todayPnl / (marketValue − todayPnl)`，见 `portfolioTotals()`）。这是券商的算法，也是图 3 记录里 `day_pnl_pct` 的同一式子，全站只有一个数。用当前市值当分母会**被当天自己的涨跌带着跑**：涨时低估涨幅、跌时高估跌幅（跌 1% 会显示成 −1.01%），越涨越显得少、越跌越显得惨。
- **金额一律精确到分**（2 位小数）：所有「元」金额都走 `src/format.ts` 的 `fmtYuan` / `fmtSignedYuan`，**不要**直接用 `fmtNum` / `fmtSigned` —— 后者的位数是展示偏好，金额位数是口径。份额（股/份，整数）、单价（元/份，基金惯例 3 位）、收益率（%）与「亿元」（成交额 / 两融 / 流通市值：0.01 亿 = 100 万元，分在亿位上无意义）都不适用。图表**坐标轴刻度**也不适用 —— 那是量程（`-8.2万`），量程上的分没有意义，精确值在 tooltip 与各处数字里。
...
| 只有 1 天 | 照画（就是一个点）—— 单点没有趋势可言，所以量程收紧到该值 ±15%、点径放大到 10px，并在图下注明「还看不出趋势」+ 累积方式。**0 天**（完全没有记录）才渲染引导 |
