/**
 * 本地实时行情接口核心：GET /api/spot（另有 /api/holdings、/api/news、/api/data-version）
 *
 * 浏览器直连新浪 hq.sinajs.cn 会被 CORS 挡住，因此由服务端做一层代理：
...
/** root + 目录（支持绝对/相对、去掉多余斜杠）+ 文件名 → 该文件的绝对路径。宿主用它构造候选路径。 */
export function dataFile(root: string, dir: string, name = 'data.json'): string {
...
/**
 * 只按结构声明用得到的字段（Connect 与 node:http 的 req/res 都满足此形状）。
 */
...
/**
 * API 中间件：与 Connect / node:http 事件处理同形。
 * `next()` 只在 pathname 不属于 /api/* 时调用，其余路径交给宿主的静态文件服务。
 */
...
export function createApiMiddleware(dataFiles: string[], holdingsFiles: string[]): ApiMiddleware {
